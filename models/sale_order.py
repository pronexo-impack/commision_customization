...
class SaleOrder(models.Model):
    ...
    def _compute_commission_total(self):
        for record in self:
            ...
            order_line_agents = self.order_line.agent_ids
            commission_05id = self.env["commission"].search([("name","=","Commission 0.5%"),("fix_qty","=","0.5")], limit=1, order="id desc")
            
            if self.order_line.ids:
                if user_id.agent:
                    if user_id.id not in partner_id.agent_ids.ids:
                        order_line_agents.write({
                            "agent_id": user_id.id,
                            "commission_id": commission_05id
                        })
                else:
                    raise ValidationError("El comerciante no esta asignado como 'Agente'")
